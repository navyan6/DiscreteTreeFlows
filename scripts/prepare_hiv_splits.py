#!/usr/bin/env python3
"""
Prepare HIV-1 Env datasets for TreeSBM: temporal + geographic splits.

Raw: data/hiv/*.fasta (all regional dumps).
Env CDS extraction + frame QC: scripts/hiv_env_utils.py

Outputs:
  data/hiv_temporal/{train,val,test}/hivtemporal{split}_group_NNN.{fasta,csv}
  data/hiv_geo/{train,val,test}/hivgeo{split}_group_NNN.{fasta,csv}
  data/hiv_temporal/SPLIT_PROTOCOL.json
  data/hiv_geo/SPLIT_PROTOCOL.json
  data/hiv_inventory.json   (raw inventory + eligibility stats)

Temporal defaults (adjusted from Env-eligible year histogram; default
train≤2021/val2022–23/test≥2024 does NOT yield ~80/10/10 on this corpus):
  train ≤ 2014
  val   = 2015
  test  ≥ 2016
Re-check tree counts after grouping; override with --train-end-year etc.

Geographic: greedy whole-unit 80/10/10 (state if present else country),
mirroring prepare_h1n1_geo.py — no unit in two splits.

Grouping: date-contiguous windows within one geoloc unit, target ~300
leaves (kept in [min_group, group_size]), max span years configurable.
Multiple non-overlapping windows per region OK.

Downstream:
  run_all_groups.py --data-dir data/hiv_temporal/{split} --prefix hivtemporal{split}
  run_all_groups.py --data-dir data/hiv_geo/{split} --prefix hivgeo{split}
"""
...
def main():
    ...
    print("\nNext: sbatch scripts/slurm_hiv_pipeline.sh  (both splits)")
