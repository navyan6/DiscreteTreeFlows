...
#SBATCH --output=logs/ab_oas_v4_cosinehead_train_%j.log
#SBATCH --error=logs/ab_oas_v4_cosinehead_train_%j.log
#
# Recipe C: OAS TreeSBM CoSiNE-style RateHeads (site CNN), parent→child CE,
# β=0, ESM-2 R0 mix init ~0, no Recipe A SHM, no PSSM gate, no Thrifty Q0.
# Does NOT overwrite ab_oas_1m_v1, v2_shm, v3_thrifty, or pathogen best.pt.
#
#   sbatch --qos=mig-max scripts/slurm_ab_oas_v4_cosinehead_train.sh

set -euo pipefail

export CKPT_DIR="${CKPT_DIR:-checkpoints/ab_oas_1m_v4_cosinehead}"
...
export R0_BACKEND="${R0_BACKEND:-esm2}"

for keep in checkpoints/ab_oas_1m_v1 checkpoints/ab_oas_1m_v2_shm \
            checkpoints/ab_oas_1m_v3_thrifty checkpoints/best; do
...
n_r0=$(ls -1 "$DATA/train"/group_*_ref_rates.pt 2>/dev/null | wc -l | tr -d ' ')
if [[ "$n_r0" -le 0 ]]; then
  echo "ERROR: no group_*_ref_rates.pt under $DATA/train (ESM Q0 caches)" >&2
  exit 1
fi
...
if [[ "$FITNESS_BETA" != "0" && "$FITNESS_BETA" != "0.0" ]]; then
  echo "REFUSING fitness β>0 for Recipe C. Got FITNESS_BETA=$FITNESS_BETA" >&2
  exit 2
fi

echo "=== Ab OAS v4 cosine mut-head TreeSBM train ==="
echo "ckpt=$CKPT_DIR r0=$R0_BACKEND esm_caches=$n_r0"
