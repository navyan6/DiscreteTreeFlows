...
#
# Usage: sbatch scripts/slurm_smoke_eval.sh <checkpoint> <data_dir> <max_seq_len> [group]
#   covid: sbatch scripts/slurm_smoke_eval.sh checkpoints/covid_v2_entropy/best.pt data/covid/test 1280 1
#   h1n1:  sbatch scripts/slurm_smoke_eval.sh checkpoints/h1n1_v1/best.pt         data/h1n1/test  566  1

export PATH=/vast/home/n/nnori/.conda/envs/treesbm/bin:$PATH
PYTHON=/vast/home/n/nnori/.conda/envs/treesbm/bin/python

CKPT="${1:?usage: sbatch slurm_smoke_eval.sh <checkpoint> <data_dir> <max_seq_len> [group]}"
DATA="${2:?usage: sbatch slurm_smoke_eval.sh <checkpoint> <data_dir> <max_seq_len> [group]}"
MSL="${3:?usage: sbatch slurm_smoke_eval.sh <checkpoint> <data_dir> <max_seq_len> [group]}"
GROUP="${4:-1}"

cd ~/DiscreteTreeFlows
mkdir -p logs

echo "Start: $(date)  ckpt=$CKPT  data=$DATA  max_seq_len=$MSL  group=$GROUP"
