#!/usr/bin/env python3
"""
Evaluate VOC threat recovery on generated (or observed) leaf sets.

Metrics (per VOC):
  voc_site_recall          — fraction of signature sites with any AA change vs root/Wuhan
  voc_exact_mut_recall     — fraction of exact signature substitutions present on ≥1 leaf
  voc_acquired_mut_recall  — same, requiring change from the tree root
  voc_bundle_any / frac    — ≥1 leaf has defining_bundle (≥ bundle_min_hits)
  topk_rbd / topk_union    — signature union on K closest leaves (optional RBD Hamming)
  evescape on recovered muts

Examples:
  # Score existing Gamma gens
  python scripts/eval_voc_threat_recovery.py \\
    --voc Gamma \\
    --gen-fasta results/covid_tree_viz/group_003_generated_matched.fasta \\
    --obs-fasta results/covid_tree_viz/group_003_observed_anc_aa.fasta \\
    --out results/voc_threat_panel/eval_gamma_g003.json

  # Top-10 by RBD Hamming to observed Gamma consensus tip
  python scripts/eval_voc_threat_recovery.py --voc Gamma \\
    --gen-fasta results/covid_tree_viz/group_003_generated_matched.fasta \\
    --obs-fasta results/covid_tree_viz/group_003_observed_anc_aa.fasta \\
    --ref-tip MZ397166.1 --topk 10 --out results/voc_threat_panel/eval_gamma_topk.json
"""
