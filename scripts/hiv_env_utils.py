#!/usr/bin/env python3
"""
Shared HIV Env helpers for TreeSBM prep.

Raw GenBank-style headers (pipe-delimited, 7 fields typical):
  >ACC |description|organism|LOCATION|COUNTRY|DATE|LENGTH

LOCATION may be country-only ("USA") or "Country: State, City".
Group unit = state (finest admin before comma) when present, else country.
Never mix regions in one tree.

Env CDS handling (documented, no guessing on ambiguous ORFs):
  - Keep near-complete Env NT CDS only: len in [MIN_ENV_NT, MAX_ENV_NT].
  - Prefer records whose description mentions envelope / (env).
  - Frame QC: require a translation that yields Env-like AA
    (N-term MRV*/MKV*/MRG*/MQP* signal peptide OR classic VENLWVTVYY mature
     start after finding an in-frame ATG), truncated AA length in
    [MIN_ENV_AA, MAX_ENV_AA], few early stops.
  - Store the in-frame NT slice starting at the chosen ATG (or frame-0 CDS
    when already starting at Env ATG) so run_all_groups nt_to_aa works.
  - Empty dates dropped. Range dates like "2015/2018" dropped (not mid-pointed).
  - Genomes: optional Env ORF extract via ATG scan + Env-like AA score; if
    no confident hit, skip (do not invent coordinates).

HXB2 Env V1–V5 (1-based Env AA, Los Alamos HXB2 numbering):
...
def geo_unit(location: str, country: str) -> tuple[str, bool]:
    ...
    else country. Do not invent states from free text without a colon.
    """
